...
use super::{
    ...
    parse_comparison_operator, parse_int_value, parse_value,
};
...
fn parse_comparison_operation(input: &mut &str) -> Result<ComparisonOperation> {
    ...
    let rhs = parse_value.parse_next(input)?;
...
mod tests {
    ...
    fn test_parse_valid_comparison_requirement() {
        ...
        assert_eq!(
            requirement.lhs,
            TypedValue::Memory(MemoryRef::new(MemorySize::Bits16, 0x1234))
        );
